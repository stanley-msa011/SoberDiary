		...
		while (startDate.before(curDate)) {
			...
			if (rand_num < 50) {
				...
				if (rand_timeslot == 0)
					...
					night = new Detection(brac, ts + NIGHT_TIMESTAMP, rand.nextInt(2), 5 + rand.nextInt(3),
							true, 0, 0);
				...
				float brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection morning = new Detection(brac, ts + MORNING_TIMESTAMP, rand.nextInt(2), 5 + rand.nextInt(3), true,
						0, 0);
				brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection afternoon = new Detection(brac, ts + AFTERNOON_TIMESTAMP, rand.nextInt(2), 5 + rand.nextInt(3),
						true, 0, 0);
				brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection night = new Detection(brac, ts + NIGHT_TIMESTAMP, rand.nextInt(2), 5 + rand.nextInt(3),
						true, 0, 0);
				int rand_timeslot = rand.nextInt(3);
				if (rand_timeslot != 0){
					db.insertDetection(morning, false);
