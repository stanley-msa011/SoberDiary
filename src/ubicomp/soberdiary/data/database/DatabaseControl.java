...
public class DatabaseControl {
    ...
    public Float[] getTodayPrimeBrac(){
    	...
    	String sql = "SELECT brac,timeSlot FROM Detection WHERE year = "+year +
    			" AND month = "+month+
    			" AND day = "+day+
    			" AND isPrime = 1"+
    ...
    public boolean detectionIsDone(){
    	...
    	String sql = "SELECT id FROM Detection WHERE" +
    			" year =" + tv.year + 
    			" AND month = " + tv.month+
    			" AND day= " + tv.day + 
    			" AND timeSlot= " + tv.timeslot;
    	Cursor cursor = db.rawQuery(sql, null);
    ...
    public boolean canTryAgain(){
    	...
    	db = dbHelper.getReadableDatabase();
    	String sql = "SELECT * FROM DETECTION WHERE year="+year+" AND month="+month+" AND day="+day+" AND timeSlot="+timeslot;
    	Cursor cursor = db.rawQuery(sql, null);
    ...
    public void updateRank(Rank data){
		...
		if (cursor.getCount() == 0) {
			...
			sql = "UPDATE Ranking SET" +
							...
							" advice_score = "+data.advice+","+
							" manage_score="+data.manage+","+
							" story_score = "+data.story+","+
							" advice_questionnaire="+data.advice_questionnaire+","+
							" advice_emotion_diy="+data.advice_emotion_diy+","+
							" manage_voice="+data.manage_voice+","+
							" manage_emotion="+data.manage_emotion+","+
							" manage_additional="+data.manage_additional+","+
							" story_read="+data.story_read+","+
							" story_test="+data.story_test+","+
							" story_fb="+data.story_fb+
    ...
    public EmotionManagement[] getDayEmotionManagement(int rYear,int rMonth,int rDay){
		...
		sql = "SELECT * FROM EmotionManagement WHERE recordYear = "+rYear+" AND recordMonth = "+rMonth+" AND recordDay = "+rDay+" ORDER BY id DESC";
		cursor = db.rawQuery(sql, null);
		int count = cursor.getCount();
		if (count == 0){
			cursor.close();
    ...
    public boolean hasEmotionManagement(TimeValue tv){
		...
		sql = "SELECT * FROM EmotionManagement WHERE" +
				" recordYear ="+tv.year+
				" AND recordMonth="+tv.month+
    ...
    public UserVoiceRecord getLatestUserVoiceRecord(){
		...
		if (!cursor.moveToFirst()){
			...
			Calendar cal =Calendar.getInstance();
			cal.setTimeInMillis(0);
			return new UserVoiceRecord(0,cal.get(Calendar.YEAR),cal.get(Calendar.MONTH),cal.get(Calendar.DAY_OF_MONTH),0);
		}
		long ts = cursor.getLong(4);
    ...
    public boolean hasAudio(TimeValue tv){
		...
		sql = "SELECT * FROM UserVoiceRecord WHERE" +
				" recordYear ="+tv.year+
				" AND recordMonth="+tv.month+
				" AND recordDay ="+tv.day;
		cursor = db.rawQuery(sql, null);
